(function () {
  ...
  async function loadGlobalAccessState() {
    try {
      const savedEndpoint = String(
        localStorage.getItem(LS.registrationEndpoint) || "",
      ).trim();
      const endpoint =
        (/script\.google\.com/i.test(savedEndpoint)
          ? DEFAULT_REGISTRATION_ENDPOINT
          : savedEndpoint) || DEFAULT_REGISTRATION_ENDPOINT;
